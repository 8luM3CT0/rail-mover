...
function EazeMoUsers({userId}) {
  const [user] = useAuthState(creds)

  const [userDoc, userLoading, userError] = useDocument(
    store?.collection('eazemo_users').doc(userId)
  )

  const giveUserAdminPriv = e => {
    e.preventDefault
        store.collection('eazemo_admins').doc(userId).set({
            isAdmin: true,
            adminName: userDoc?.data()?.displayName,
            adminEmail: userDoc?.data()?.email,
            adminPic: userDoc?.data()?.photoURL
        }, {
            merge: true
        }).then(() => {
          store.collection('eazemo_users').doc(userId).delete()
        })
    }
    ...
    className='w-[80%] mx-auto min-h-[120px] max-h-[150px] flex items-center rounded bg-slate-200 border-2 border-slate-800 hover:bg-slate-300 hover:border-slate-600 transform transition-all duration-300 ease-in-out cursor-pointer'>
      ...
      <span className="w-[35%] h-full flex items-center space-x-2 px-3 py-1">
        <button onClick={giveUserAdminPriv} className="flex flex-col justify-center items-center h-[60px] w-[60px] rounded border-2 border-slate-800 hover:border-slate-500">
          <PersonAddIcon style={{
            fontSize: '1.3em'
          }} />
        </button>
