...
// ----------------- COMPONENT -----------------
function EzPocketSidebar({ onFocusStation, onRouteFound, stationsOptions, setStationsOptions }) {
  ...
  return (
    <>
      <div className='border-8 h-full w-[40%] border-slate-800 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50'>
        <header className="h-[10%] space-x-3 w-full border-b-2 border-slate-800 flex items-center px-3">
          <div className="flex flex-col space-y-1 w-full">
            <span className="text-xs font-montserr text-slate-600">
              Wallet balance
            </span>
            <span className="text-lg font-montserr font-bold text-slate-900">
              {user
                ? walletLoading
                  ? "Loading…"
                  : `₱${balance.toFixed(2)}`
        ...
        <div className="h-[90%] w-full flex flex-col space-y-2 items-start">
          {/* line filters + search */}
          <header className="w-full h-[20%] border-y-2 border-slate-800 flex flex-col items-center  px-3 py-1 space-y-2">
            <div className="flex h-[50%] items-center space-x-2">
              <button
                ...
                className={`h-[60%] min-w-[80px] rounded-md bg-inherit border border-slate-800 text-xs font-montserr text-slate-800 hover:bg-slate-300 hover:border-slate-600 hover:text-slate-600 transform transition-all duration-300 ease-in-out ${lrt1Tab ? 'bg-amber-500 border-2 border-zinc-800' : ''} `}
              >
                LRT-1
              </button>
              <button
                ...
                className={`h-[60%] min-w-[80px] rounded-md bg-inherit border border-slate-800 text-xs font-montserr text-slate-800 hover:bg-slate-300 hover:border-slate-600 hover:text-slate-600 transform transition-all duration-300 ease-in-out ${lrt2Tab ? 'bg-amber-500 border-2 border-zinc-800' : ''} `}
              >
                LRT-2
              </button>
              <button
                ...
                className={`h-[60%] min-w-[80px] rounded-md bg-inherit border border-slate-800 text-xs font-montserr text-slate-800 hover:bg-slate-300 hover:border-slate-600 hover:text-slate-600 transform transition-all duration-300 ease-in-out ${mrt3Tab ? 'bg-amber-500 border-2 border-zinc-800' : ''} `}
              >
                MRT-3
              </button>
            </div>
            <div className="flex items-center w-full h-[50%]">
              <span className="w-[55%]"></span>
              <input
                type="text"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search station…"
                className="w-[45%] h-[60%] max-w-[180px] px-2 py-1 text-xs border border-slate-800 rounded bg-white text-slate-800 font-montserr"
              />
            </div>
          </header>
          ...
          <div className="h-[80%] w-full my-auto overflow-y-scroll scrollbar-thin scrollbar-track-slate-800 scrollbar-thumb-slate-100 border-2 border-slate-800">
            <>
              <div className="px-4 py-3">
                <div className="mt-2 min-h-[240px] max-h-[280px] overflow-y-scroll scrollbar-thin scrollbar-track-slate-800 scrollbar-thumb-slate-200 border border-slate-800 rounded bg-slate-600 bg-opacity-[0.12]">
                  {stationsLoading && (
              ...
              {showQR && (
                <div className="absolute top-20 bottom-24 left-0 right-20  min-h-[420px] max-h-[500px] min-w-[360px] max-w-[390px] flex flex-col items-center bg-slate-800 p-3 rounded shadow border">
                ...
                <div className="mt-2 px-2 w-full max-h-[220px] flex flex-col gap-2">
                  ...
                  <div className="flex-1 w-full max-h-[120px] overflow-y-auto scrollbar-thin scrollbar-track-slate-800 scrollbar-thumb-slate-600 border border-amber-400/40 rounded px-2 py-1">
                    {guideLoading ? (
                      ...
                      {overlayMode === "trace" && (
                        <div className="mt-2 px-2 h-[92%] w-[220px] max-h-[220px] flex flex-col gap-2">
                      ...
                      {overlayMode === "geo-trace" && (
                        <div className="mt-2 px-2 w-full max-h-[220px] flex flex-col gap-2">
                          <div className="w-full px-3 flex items-start">
                            <h3 className="font-stack-head font-semibold text-amber-300 text-[12px]">
                              Geo-Trace Mode
                            </h3>
                          </div>
                        </div>
...
export default EzPocketSidebar
