...
import TravelHistoryCard from './travel-history-card/TravelHistoryCard'
//back-end
...
function Sidebar({onFocusStation, onRouteFound, subscribeModal, setSubscribeModal}) {
...
const checkUserIfSubscribed = async() => {
  ...
  const stationQuery = useMemo(() => {
    ...
    try{
      return qq 
      ? baseStationsRef?.orderBy("stationNameLower").startAt(qq).endAt(qq + "\uf8ff").limit(40)
      : baseStationsRef?.orderBy("stationNameLower").limit(40)
    }catch{
      return baseStationsRef.limit(40)
    }
  }, [baseStationsRef])
  ...
  return (
    <>
    <div className='mobileDisplay border-r-2 h-full w-[30%] border-slate-800 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50'>
      <header className="h-[10%] w-full border-b-2 border-slate-800 flex items-center"></header>
      <div className="h-[90%] w-full flex flex-col space-y-2 items-start">
        {/**top div has search option */}
        <div className="h-[48%] w-full border-2 border-slate-800">
          {user ? (
            <>
            <div className="px-4 py-3">
              
                            <div className="mt-2 min-h-[100px] max-h-[120px] overflow-auto scrollbar-thin scrollbar-track-slate-800 scrollbar-thumb-slate-200 border border-slate-800 rounded bg-slate-600 bg-opacity-[0.12]">
            ...
            <div key={s.id} className="flex items-center justify-between px-3 py-2 border-b last:border-b-0">
              ...
              {(isUserASub || isUserAnEditor) && (
                ...
                <button
                  ...
                  onClick={() => onFocusStation({ lat: s.lat, lng: s.lng, name: s.name })}
                >
                  Focus
                </button>
                <button
      ...
      {/*nfcMsg && (
        <div className="absolute bottom-24 right-8 bg-white text-xs p-2 rounded shadow border">
          {nfcMsg}
        </div>
      )*/}
       </div>
        ...
        <div className="h-[48%] w-full border-2 border-slate-800">
          {user ? (
           ...
           <div className="h-full w-full flex flex-col items-start">
            <header className="h-[8%] w-full flex items-center px-3 py-1  border-b-2 border-slate-800 ">
              <h3 className="font-montserr font-semibold text-slate-800 text-sm">
                Travel history
              </h3>
            </header>
            <div className="h-[92%] w-full flex flex-col items-center px-3 py-1 space-y-6 overflow-y-scroll scrollbar-thin scrollbar-track-slate-800 scrollbar-thumb-slate-100">
            {!userTravelHistory?.docs ? (
              <h2 className="font-montserr font-bold text-xl text-slate-800">
                Travel history N/A
              </h2>  
          ): userTravelHistory?.docs?.map((travelId) => (
            <TravelHistoryCard />
          ))}
            </div>
           </div>
